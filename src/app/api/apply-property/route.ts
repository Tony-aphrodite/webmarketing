  ...
  try {
    ...
    if (process.env.RESEND_API_KEY) {
      ...
      resend.emails.send({
        ...
        `,
      }).catch((err) => console.error("Commercial application email failed:", err));

      // Steve 4/24 #6: 2. Confirmation email to tenant (was missing)
      const tenantEmail = user.email || profile?.email;
      console.log(`[apply-property] Sending confirmation to tenant: ${tenantEmail}`);
      if (tenantEmail) {
        resend.emails.send({
          from: FROM_EMAIL,
          to: [tenantEmail],
  ...
  <div style="background:#fff;padding:24px;border:1px solid #e5e5e5;border-top:none">
    ...
    <p style="margin-top:32px;padding-top:16px;border-top:1px solid #e5e5e5;font-size:12px;color:#888;text-align:center">
      Nexuma marketing ltd
        ...
        }).catch((err) => console.error("Tenant confirmation email failed:", err));
      }
    }
